...
public class JwtActions {
  ...
  @Value("${gatishakti.app.jwtSecret}")
  ...
  @Value("${gatishakti.app.jwtExpirationMs}")
  private int jwtExpirationMs;

  public String generate(Jwt jwt) {
    return Jwts.builder().subject(jwt.subject()).claim("fp", jwt.fp())
        .issuedAt(new Date(jwt.timestamp())).expiration(new Date(jwt.timestamp() + jwtExpirationMs))
        .encryptWith(secretKey, Jwts.KEY.PBES2_HS512_A256KW, Jwts.ENC.A256GCM).compact();
  }
  ...
  public Optional<Jwt> validate(String token, String fpFromRequest) {
    ...
    try {
      Claims claims =
          Jwts.parser().decryptWith(secretKey).build().parseEncryptedClaims(token).getPayload();
